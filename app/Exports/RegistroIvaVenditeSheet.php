...
class RegistroIvaVenditeSheet implements FromQuery, WithHeadings, WithMapping, ShouldAutoSize, WithStyles, WithTitle
{
    public function __construct(private readonly int $anno) {}

    public function title(): string { return 'Registro Vendite'; }

    public function query()
    {
        return FatturaAttiva::query()
            ->whereYear('data_fattura', $this->anno)
            ->where('stato', '!=', FatturaAttiva::STATO_ANNULLATA)
            ->orderBy('data_fattura');
    }

    public function headings(): array
    {
        return ['Data Fattura', 'N° Fattura', 'Tipo Documento', 'Imponibile €', 'IVA €', 'Totale €', 'Stato'];
    }
