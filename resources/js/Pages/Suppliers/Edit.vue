...
const form = useForm({
    ...
    ragione_sociale:      props.supplier.ragione_sociale     ?? '',
    email:                props.supplier.email               ?? '',
    phone:                props.supplier.phone               ?? '',
    partita_iva:          props.supplier.partita_iva         ?? '',
    codice_fiscale:       props.supplier.codice_fiscale      ?? '',
    codice_sdi:           props.supplier.codice_sdi          ?? '',
    pec:                  props.supplier.pec                 ?? '',
    indirizzo:            props.supplier.indirizzo           ?? '',
    cap:                  props.supplier.cap                 ?? '',
    citta:                props.supplier.citta               ?? '',
    provincia:            props.supplier.provincia           ?? '',
    nazione:              props.supplier.nazione             ?? 'IT',
    iban:                 props.supplier.iban                ?? '',
    condizioni_pagamento: props.supplier.condizioni_pagamento ?? '30gg',
    categoria:            props.supplier.categoria           ?? 'servizi',
    note:                 props.supplier.note                ?? '',
    attivo:               props.supplier.attivo              ?? true,
});
</script>

<template>
    <AppLayout :title="`Modifica – ${supplier.ragione_sociale || supplier.name}`">
        <Head :title="`Modifica – ${supplier.ragione_sociale || supplier.name}`" />
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="route('suppliers.show', supplier.id)" class="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
                    <ArrowLeftIcon class="size-5" aria-hidden="true" />
                </Link>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    Modifica — {{ supplier.ragione_sociale || supplier.name }}
                </h2>
            </div>
        </template>

        <div class="py-6 max-w-4xl mx-auto sm:px-6">
            <form @submit.prevent="form.put(route('suppliers.update', supplier.id))" class="space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <TextInput id="partita_iva" v-model="form.partita_iva" class="mt-1 block w-full font-mono" maxlength="20" />
                        ...
                        <div>
                            ...
                            <TextInput id="codice_sdi" v-model="form.codice_sdi" class="mt-1 block w-full font-mono" maxlength="7" />
                            <InputError class="mt-1" :message="form.errors.codice_sdi" />
                        </div>
                        <div>
                            <InputLabel for="pec" value="PEC" />
                            <TextInput id="pec" v-model="form.pec" type="email" class="mt-1 block w-full" />
                ...
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <TextInput id="provincia" v-model="form.provincia" class="mt-1 block w-full font-mono uppercase" maxlength="2" />
                            <InputError class="mt-1" :message="form.errors.provincia" />
                        </div>
                        <div>
                            <InputLabel for="nazione" value="Nazione (ISO)" />
                            <TextInput id="nazione" v-model="form.nazione" class="mt-1 block w-full font-mono uppercase" maxlength="2" />
                            <InputError class="mt-1" :message="form.errors.nazione" />
                        </div>
                    </div>
                </div>

                <!-- Dati bancari e condizioni -->
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div class="sm:col-span-2">
                            ...
                            <TextInput id="iban" v-model="form.iban" class="mt-1 block w-full font-mono" maxlength="34" />
