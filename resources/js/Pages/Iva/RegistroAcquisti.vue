...
function applyFilters() {
    router.get(
        route('iva.registro-acquisti', { tenant: tenant.value?.slug }),
...
const statoBadge = {
    da_pagare:     'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-300',
    pagata:        'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300',
    parzialmente_pagata: 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300',
    scaduta:       'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300',
};
const statoLabel = {
    da_pagare:           'Da pagare',
    pagata:              'Pagata',
...
<template>
    <AppLayout title="Registro Acquisti IVA">
        <Head title="Registro Acquisti IVA" />
        ...
        <template #header>
            <div class="flex justify-between items-center flex-wrap gap-2">
                ...
                <a
                    :href="route('iva.registro-acquisti', { tenant: tenant?.slug }) + '?anno=' + form.anno + '&periodo=' + form.periodo + '&tipo_periodo=' + form.tipo_periodo + '&export=csv'"
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-4 text-center">
                        ...
                        <p class="mt-1 text-xl font-semibold text-indigo-600 dark:text-indigo-400">{{ fmtEur(totali?.iva) }}</p>
                    </div>
                    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-4 text-center">
                ...
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead class="bg-gray-50 dark:bg-gray-700">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Data Reg.</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">N° Fattura</th>
                                <th class="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase">Tipo Doc.</th>
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                            <template v-for="f in fatture.data" :key="f.id">
                                ...
                                >
                                    ...
                                    <td class="px-4 py-2 text-sm whitespace-nowrap">{{ fmtData(f.data_registrazione) }}</td>
                                    <td class="px-4 py-2 text-sm font-medium">{{ f.numero_fattura || '—' }}</td>
                                    <td class="px-4 py-2 text-sm text-gray-500 dark:text-gray-400">{{ f.tipo_documento || '—' }}</td>
                                    <td class="px-4 py-2 text-sm text-right whitespace-nowrap">{{ fmtEur(f.imponibile_totale) }}</td>
                                    <td class="px-4 py-2 text-sm text-right whitespace-nowrap text-indigo-600 dark:text-indigo-400">{{ fmtEur(f.iva_totale) }}</td>
                                    <td class="px-4 py-2 text-sm text-right font-medium whitespace-nowrap">{{ fmtEur(f.totale_documento) }}</td>
                                    <td class="px-4 py-2">
                                        <span
                                            class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium"
                                            :class="statoBadge[f.stato_pagamento] ?? 'bg-gray-100 text-gray-700'"
                                        >
                                            {{ statoLabel[f.stato_pagamento] ?? f.stato_pagamento ?? '—' }}
                                        </span>
                                    </td>
                                </tr>
                                <!-- Righe IVA espanse -->
                                <tr v-if="expanded.has(f.id) && f.righe?.length" class="bg-indigo-50/50 dark:bg-indigo-900/10">
                                        ...
                                        <table class="w-full text-xs">
                                            <thead>
                                                <tr class="text-gray-500 dark:text-gray-400">
                                                    ...
                                                    <th class="text-right pb-1 font-medium">IVA Indeduc.</th>
                                                </tr>
                                            </thead>
                                            <tbody class="divide-y divide-indigo-100 dark:divide-indigo-900/30">
                                                <tr v-for="r in f.righe" :key="r.id">
                                                    ...
                                                    <td class="py-0.5 text-right text-indigo-600 dark:text-indigo-400">{{ fmtEur(r.iva) }}</td>
                                                    <td class="py-0.5 text-right text-red-500">{{ fmtEur(r.iva_indetraibile) }}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </td>
                    ...
                    <p v-if="!fatture.data?.length" class="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                        Nessuna fattura passiva nel periodo selezionato.
                    </p>
