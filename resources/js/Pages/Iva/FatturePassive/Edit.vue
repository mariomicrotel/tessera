...
/* ── Helper per formattare date ISO → input[type=date] ──────────────── */
const toInputDate = (d) => d ? d.substring(0, 10) : '';

/* ── Righe esistenti → formato form ─────────────────────────────────── */
const righeInit = props.fattura.righe?.map(r => ({
    codice_iva_id:            r.codice_iva_id,
    conto_id:                 r.conto_id ?? null,
    descrizione:              r.descrizione,
    quantita:                 Number(r.quantita),
    prezzo_unitario:          Number(r.prezzo_unitario),
    indetraibile_percentuale: r.indetraibile_percentuale !== null ? Number(r.indetraibile_percentuale) : null,
})) ?? [rigaVuota()];

/* ── Form ─────────────────────────────────────────────────────────────── */
const form = useForm({
    supplier_id:         props.fattura.supplier_id ?? null,
    numero_fattura:      props.fattura.numero_fattura ?? '',
    data_fattura:        toInputDate(props.fattura.data_fattura),
    data_ricezione:      toInputDate(props.fattura.data_ricezione),
    data_registrazione:  toInputDate(props.fattura.data_registrazione),
    data_scadenza:       toInputDate(props.fattura.data_scadenza),
    esigibilita:         props.fattura.esigibilita ?? 'immediata',
    tipo_documento:      props.fattura.tipo_documento ?? 'TD01',
    ...
    <AppLayout :title="`Modifica – ${fattura.numero_fattura}`">
        ...
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="route('iva.fatture-passive.show', fattura.id)"
                      class="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300">
                    <ArrowLeftIcon class="size-5" aria-hidden="true" />
                </Link>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    Modifica fattura — <span class="font-mono">{{ fattura.numero_fattura }}</span>
                </h2>
            </div>
        </template>

        <div class="py-6 max-w-6xl mx-auto sm:px-6">
            <form @submit.prevent="form.put(route('iva.fatture-passive.update', fattura.id))" class="space-y-6">
