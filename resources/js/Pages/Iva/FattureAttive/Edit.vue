...
/* ── Form prefilled ──────────────────────────────────────────────────────── */
const form = useForm({
    cliente_id:          props.fattura.cliente_id,
    data_fattura:        props.fattura.data_fattura?.slice(0, 10) ?? '',
    data_scadenza:       props.fattura.data_scadenza?.slice(0, 10) ?? '',
    esigibilita:         props.fattura.esigibilita,
    note:                props.fattura.note ?? '',
    righe: props.fattura.righe.map(r => ({
...
function submit() {
    form.put(route('iva.fatture-attive.update', props.fattura.id));
}
</script>

<template>
    <AppLayout :title="`Modifica ${fattura.numero_fattura}`">
        <Head :title="`Modifica fattura ${fattura.numero_fattura}`" />
        <template #header>
            <div class="flex items-center gap-3">
                ...
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    Modifica fattura
                    <span class="font-mono text-indigo-600 dark:text-indigo-400 ml-1">{{ fattura.numero_fattura }}</span>
                </h2>
            </div>
        </template>
