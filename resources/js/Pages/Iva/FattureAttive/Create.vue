...
function rimuoviRiga(index) {
    if (form.righe.length > 1) form.righe.splice(index, 1);
}

/* ── Calcolo live ────────────────────────────────────────────────────────── */
function getAliquota(codiceIvaId) {
    const c = props.codiciIva.find(c => c.id === Number(codiceIvaId));
    return c ? Number(c.percentuale) : 0;
}

function calcolaRiga(riga) {
    const qta     = Number(riga.quantita         ?? 0);
    const prezzo  = Number(riga.prezzo_unitario  ?? 0);
    const scontoPct = Number(riga.sconto_percentuale ?? 0);
    const lordo   = Math.round(qta * prezzo * 100) / 100;
    const sconto  = Math.round(lordo * scontoPct / 100 * 100) / 100;
    const imp     = Math.round((lordo - sconto) * 100) / 100;
    const aliq    = getAliquota(riga.codice_iva_id);
    const iva     = Math.round(imp * aliq / 100 * 100) / 100;
    return { imp, iva, totale: Math.round((imp + iva) * 100) / 100 };
}
...
function submit() {
    form.post(route('iva.fatture-attive.store'));
}
</script>

<template>
    <AppLayout title="Nuova fattura attiva">
        <Head title="Nuova fattura attiva" />
        <template #header>
        ...
        <div class="py-6 max-w-6xl mx-auto sm:px-6">
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <select id="cliente_id" v-model="form.cliente_id"
                                ...
                                <option v-for="c in clienti" :key="c.id" :value="c.id">
                                    ...
                                    <template v-if="c.codice_fiscale"> ({{ c.codice_fiscale }})</template>
                                </option>
                            </select>
                            <InputError class="mt-1" :message="form.errors.cliente_id" />
                        </div>
                        ...
                        <div>
                            ...
                            <select id="stato" v-model="form.stato" required
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm text-sm">
                                <option value="bozza">Bozza</option>
                                <option value="emessa">Emessa</option>
                            </select>
                            <InputError class="mt-1" :message="form.errors.stato" />
                        </div>
                        ...
                        <div>
                            <InputLabel for="anno" value="Anno *" />
                            <TextInput id="anno" v-model="form.anno" type="number" class="mt-1 block w-full" required />
                            <InputError class="mt-1" :message="form.errors.anno" />
                        </div>
                ...
                <!-- ── Conti contabili (opzionale) ─────────────────────── -->
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 space-y-4">
                    <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100 border-b dark:border-gray-700 pb-2">
                        Conti contabili
                        <span class="text-xs font-normal text-gray-400 ml-2">(opzionale — necessari per generare la scrittura contabile)</span>
                    </h3>
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            <InputLabel for="conto_crediti_id" value="Conto Crediti (Attivo)" />
                            <select id="conto_crediti_id" v-model="form.conto_crediti_id"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm text-sm">
                                <option :value="null">— Nessuno —</option>
                                <option v-for="c in contiAttivo" :key="c.id" :value="c.id">{{ c.codice }} – {{ c.descrizione }}</option>
                            </select>
                            <InputError class="mt-1" :message="form.errors.conto_crediti_id" />
                        </div>
                        <div>
                            <InputLabel for="conto_ricavi_id" value="Conto Ricavi" />
                            <select id="conto_ricavi_id" v-model="form.conto_ricavi_id"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm text-sm">
                                <option :value="null">— Nessuno —</option>
                        ...
                        <div>
                            <InputLabel for="conto_iva_debito_id" value="Conto IVA a debito" />
                            <select id="conto_iva_debito_id" v-model="form.conto_iva_debito_id"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 shadow-sm text-sm">
                                <option :value="null">— Nessuno —</option>
                ...
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6 space-y-4">
                        ...
                        <div v-for="(riga, idx) in form.righe" :key="idx"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-6 gap-3">
                                ...
                                <div class="sm:col-span-1">
                                    <InputLabel :for="`qta_${idx}`" value="Qtà *" />
                                ...
                                <!-- Sconto % -->
                                <div class="sm:col-span-1">
                                    <InputLabel :for="`sconto_${idx}`" value="Sconto %" />
                                    <TextInput :id="`sconto_${idx}`" v-model="riga.sconto_percentuale"
                                        type="number" step="0.1" min="0" max="100"
                                        class="mt-1 block w-full font-mono text-right" placeholder="0" />
                                    <InputError class="mt-1" :message="form.errors[`righe.${idx}.sconto_percentuale`]" />
                                </div>

                                <!-- Totali calcolati riga (read-only) -->
                                <div class="sm:col-span-6 flex flex-wrap items-center gap-6 text-sm text-gray-600 dark:text-gray-400">
                                    ...
                                    <div class="text-right">
                                        <div class="text-xs text-gray-400">IVA ({{ getAliquota(riga.codice_iva_id) }}%)</div>
                    ...
                    <div class="border-t dark:border-gray-700 pt-4">
                        <div class="flex justify-end">
                            <dl class="text-sm space-y-1 min-w-52">
                ...
                <div class="flex gap-3">
                    <PrimaryButton type="submit" :disabled="form.processing">
                        <CheckIcon class="size-4 me-2" aria-hidden="true" />Salva fattura
                    </PrimaryButton>
                    <Link :href="route('iva.fatture-attive.index')"
