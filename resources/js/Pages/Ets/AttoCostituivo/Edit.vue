...
const form = useForm({
    notaio:                props.atto.notaio ?? '',
    repertorio:            props.atto.repertorio ?? '',
    data_atto:             props.atto.data_atto ?? '',
    data_registrazione_ae: props.atto.data_registrazione_ae ?? '',
    ufficio_registro:      props.atto.ufficio_registro ?? '',
    numero_registro:       props.atto.numero_registro ?? '',
    stato:                 props.atto.stato,
    note:                  props.atto.note ?? '',
});

const submit = () => form.put(route('ets.atto-costitutivo.update', props.atto.id));
</script>

<template>
    <AppLayout title="Modifica Atto Costitutivo">
        <Head title="Modifica Atto Costitutivo" />
        <template #header>
            <div class="flex items-center gap-3">
                <Link :href="route('ets.atto-costitutivo.show', atto.id)" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200">
                    <ArrowLeftIcon class="size-5" />
                </Link>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200">Modifica Atto Costitutivo</h2>
            </div>
        </template>
                    ...
                    <div class="bg-white dark:bg-gray-800 shadow rounded-lg px-6 py-6 space-y-4">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input v-model="form.notaio" type="text" maxlength="255" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200 shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                        ...
                        <div class="flex justify-end gap-3 pt-2">
                            <Link :href="route('ets.atto-costitutivo.show', atto.id)">
