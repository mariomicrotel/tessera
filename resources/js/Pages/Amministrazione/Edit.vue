...
const form = useForm({
    ...
    tipo:                  props.movimento.tipo ?? 'entrata',
    importo:               props.movimento.importo ?? '',
    descrizione:           props.movimento.descrizione ?? '',
    category_id:           props.movimento.category_id ?? '',
    conto_id:              props.movimento.conto_id ?? '',
    conto_destinazione_id: props.movimento.conto_destinazione_id ?? '',
    riferimento:           props.movimento.riferimento ?? '',
    note:                  props.movimento.note ?? '',
});
...
const submit = () => form.put(route('movimenti-amministrativi.update', props.movimento.id));
</script>

<template>
    <AppLayout title="Modifica Movimento">
        <Head title="Modifica Movimento Amministrativo" />
        <template #header>
            <div>
                <div class="mb-1">
                ...
                <form @submit.prevent="submit"
                    ...
                    <div v-if="form.tipo !== 'giroconto'">
                        ...
                        <select id="conto_id" v-model="form.conto_id"
                            ...
                            <option v-for="c in conti" :key="c.id" :value="c.id">{{ c.name }} ({{ c.type }})</option>
                        </select>
                    ...
                    <!-- Riferimento -->
                    <div>
                        <InputLabel for="riferimento" value="Riferimento" />
                        <TextInput id="riferimento" v-model="form.riferimento" type="text" class="mt-1 block w-full" />
                    ...
                    <div class="flex items-center justify-end gap-3 pt-2 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <PrimaryButton type="submit" :disabled="form.processing">
                            {{ form.processing ? 'Salvataggio...' : 'Aggiorna movimento' }}
                        </PrimaryButton>
                    </div>
                </form>
            </div>
